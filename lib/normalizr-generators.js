...
function generateNormalizrDefine(table) {
    ...
    buffer += table.mapClassMembers(function (hasMany) { return ("\t" + hasMany.getName() + " : arrayOf(" + helpers_1.toCamel(hasMany.getManyType().name) + "),"); }, function (hasOne) { return ("\t" + hasOne.getName() + " : " + helpers_1.toCamel(hasOne.getOneType().name) + ","); });
    buffer += "});\n";
    return buffer;
}
