...
function generateWhereInterface(modelFile, modelRoot, collectClass) {
    var buffer = "";
    if (!collectClass) {
    ...
    for (var _i = 0, _a = collectClass.properties; _i < _a.length; _i++) {
        var p = _a[_i];
        var fp = new helpers_1.EntityField(modelFile, modelRoot, collectClass, p);
        var typeName = fp.getTypeName();
        if (fp.isEnum()) {
            typeName = "Model." + typeName;
        }
