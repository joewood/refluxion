...
if (clientQl) {
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        helpers_1.appendLine(outputGraphQLClient_1, generate_interfaces_1.generateWhereInterface(modelFile, root, table.getWhereClass()));
...
if (sequelize) {
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        var buffer = table.mapEntityRelationships(function (hasMany) { return ("\t\t" + hasMany.getName() + ": Sequelize.Model<Interfaces." + hasMany.getManyTypeInterfaceName() + ",any>;"); }, function (hasOne) { return ("\t\t" + hasOne.getName() + ": Sequelize.Model<Interfaces." + hasOne.getOneInterfaceTypeName() + ",any>;"); });
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        for (var _i = 0, _a = table.getTableType().properties; _i < _a.length; _i++) {
            ...
            if (field.kind !== TsTypeInfo.ClassPropertyKind.Normal) {
                continue;
            }
            var primaryKey = (field.name === "ID") ? ", primaryKey:true " : "";
            helpers_1.appendLine(outputSequelize_1, "\t\t\t" + field.name + ": { type: " + (generate_sequelize_types_1.getSequelizeTypeofProp(modelFile, root, table.getTableType(), field) + primaryKey) + " },");
        }
        helpers_1.appendLine(outputSequelize_1, "\t\t}),");
    ...
    helpers_1.appendLine(outputSequelize_1, "\tconst hasManyOptions = { constraints:false, foreignKeyConstraint:false, onUpdate:\"NO ACTION\", onDelete:\"SET NULL\"};");
    helpers_1.iterateRoot(modelFile, root, function (table) {
        if (!table.isTable) {
            return;
        }
        ...
        var buffer = table.mapEntityRelationships(function (hasMany) { return ("\ttables." + table.getTableName() + ".hasMany(tables." + hasMany.getManyTableName() + ",\n                Object.assign({},hasManyOptions, { foreignKey: \"" + hasMany.property.name + "\", as: \"" + hasMany.getName() + "\"} ));"); }, function (hasOne) { return ("\ttables." + table.getTableName() + ".belongsTo(tables." + hasOne.getOneTableName() + ",\n                Object.assign({},hasManyOptions, { foreignKey: \"" + hasOne.property.name + "\", as: \"" + hasOne.getName() + "\"} ));"); });
        helpers_1.appendLine(outputSequelize_1, buffer);
    });
    helpers_1.appendLine(outputSequelize_1, "}");
}
...
if (interfaces) {
    ...
    helpers_1.appendLine(outputInterfaces_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, root, "", false));
    helpers_1.iterateRoot(modelFile, root, function (table) {
        if (!table.isTable) {
            return;
        }
        helpers_1.appendLine(outputInterfaces_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, table.getTableType(), "", false));
    });
    helpers_1.appendLine(outputInterfaces_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, root, "Lists", true));
    ...
    helpers_1.appendLine(outputPathOptional_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, root, "", false));
    helpers_1.iterateRoot(modelFile, root, function (table) {
        if (!table.isTable) {
            return;
        }
        helpers_1.appendLine(outputPathOptional_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, table.getTableType(), "", false, true));
    });
    helpers_1.appendLine(outputPathOptional_1, generate_interfaces_1.generateInterfaceForClass(modelFile, root, root, "Lists", true, true));
}
if (graphql) {
    ...
    helpers_1.appendLine(outputGraphQL_1, "import { Tables } from \"./" + outputBasename + ".sequelize\";");
    helpers_1.appendLine(outputGraphQL_1, "import * as Model from \"./" + justRelativeFilename + "\";\n");
    ...
    helpers_1.appendLine(outputGraphQL_1, "export function getGraphQL( tables: Tables ) : GraphQLTypes {");
    helpers_1.appendLine(outputGraphQL_1, "\tconst types : GraphQLTypes = {};");
    helpers_1.iterateRoot(modelFile, root, function (table) {
        if (!table.isTable) {
            return;
        }
        helpers_1.appendLine(outputGraphQL_1, graphql_generators_1.generateGraphQLAttributes(modelFile, root, table, table.getWhereClass(), table.getTableName()) + "\n");
    });
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        helpers_1.appendLine(outputGraphQL_1, graphql_generators_1.generateGraphQLArgs(modelFile, root, table, table.tableProperty, table.getWhereClass()) + "\n");
    });
    helpers_1.iterateRoot(modelFile, root, function (table) {
        if (table.isTable) {
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        helpers_1.appendLine(outputGraphQL_1, generate_interfaces_1.generateWhereInterface(modelFile, root, table.getWhereClass()));
    });
}
helpers_1.flushLines();
