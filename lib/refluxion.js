...
if (sequelize) {
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        for (var _i = 0, _a = table.getTableType().properties; _i < _a.length; _i++) {
            ...
            if (field.name === "id")
                continue;
            helpers_1.appendLine(outputSequelize_1, "\t\t\t" + field.name + ": { type: " + generate_sequelize_types_1.getSequelizeTypeofProp(modelFile, root, table.getTableType(), field) + " },");
        }
        helpers_1.appendLine(outputSequelize_1, "\t\t}),");
    ...
    helpers_1.iterateRoot(modelFile, root, function (table) {
        ...
        var buffer = table.mapEntityRelationships(function (hasMany) { return ("\ttables." + table.getTableName() + ".hasMany(tables." + hasMany.getManyTableName() + ", { as: \"" + hasMany.getName() + "\", constraints:false, foreignKeyConstraint:false, onUpdate:\"NO ACTION\", onDelete:\"SET NULL\"} )"); }, function (hasOne) { return ("\ttables." + table.getTableName() + ".belongsTo(tables." + hasOne.getOneTableName() + ", { foreignKey: \"" + hasOne.property.name + "\", as: \"" + hasOne.getName() + "\", constraints:false, foreignKeyConstraint:false, onUpdate:\"NO ACTION\", onDelete:\"SET NULL\" })"); });
        helpers_1.appendLine(outputSequelize_1, buffer);
    });
    helpers_1.appendLine(outputSequelize_1, "}");
}
